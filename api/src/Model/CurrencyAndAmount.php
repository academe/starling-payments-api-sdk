...
/**
 * CurrencyAndAmount Class Doc Comment
 *
 * @category Class
 * @package  Academe\Starling\PaymentsSdk
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 */
class CurrencyAndAmount implements ModelInterface, ArrayAccess, JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'CurrencyAndAmount';
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['minorUnits'] = $data['minorUnits'] ?? null;
    }
    ...
    public function listInvalidProperties()
    {
        ...
        if ($this->container['currency'] === null) {
            $invalidProperties[] = sprintf('"%s" can\'t be null', 'currency');
        }
        if ($this->container['minorUnits'] === null) {
            $invalidProperties[] = sprintf('"%s" can\'t be null', 'minorUnits');
        }
        if (($this->container['minorUnits'] < 0)) {
            $invalidProperties[] = sprintf(
                'invalid value for "%s", must be bigger than or equal to %d.',
                'minorUnits',
                0
            );
        }
    ...
    {
        return $this->get('minorUnits');
    }

    /**
     * Sets minorUnits
     *
     * @param int $minorUnits Amount in the minor units of the given currency; eg pence in GBP, cents in EUR
     *
     * @return $this
     */
    public function setMinorUnits($minorUnits)
    {

        if (($minorUnits < 0)) {
            throw new \InvalidArgumentException('invalid value for $minorUnits when calling CurrencyAndAmount., must be bigger than or equal to 0.');
        }

        $this->container['minorUnits'] = $minorUnits;
