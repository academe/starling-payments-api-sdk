...
/**
 * PaymentReturnRequest Class Doc Comment
 *
 * @category Class
 ...
 * @link     https://openapi-generator.tech
 */
class PaymentReturnRequest implements ModelInterface, ArrayAccess, JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'PaymentReturnRequest';
    ...
    protected static $attributeMap = [
        'paymentUidToReturn' => 'paymentUidToReturn',
        'reference' => 'reference',
        'reason' => 'reason'
    ];
    ...
    protected static $setters = [
        'paymentUidToReturn' => 'setPaymentUidToReturn',
    ...
    protected static $getters = [
        'paymentUidToReturn' => 'getPaymentUidToReturn',
    ...
    const REASON_ACCOUNT_UNKNOWN = 'ACCOUNT_UNKNOWN';
    const REASON_ACCOUNT_CLOSED = 'ACCOUNT_CLOSED';
    const REASON_ACCOUNT_STOPPED = 'ACCOUNT_STOPPED';
    const REASON_ACCOUNT_HOLDER_DECEASED = 'ACCOUNT_HOLDER_DECEASED';
    const REASON_REFERENCE_REQUIRED_NOT_SUPPLIED = 'REFERENCE_REQUIRED_NOT_SUPPLIED';
    const REASON_ACCOUNT_NAME_MISSMATCH = 'ACCOUNT_NAME_MISSMATCH';
    const REASON_RETURN_REQUESTED_BY_SENDER = 'RETURN_REQUESTED_BY_SENDER';
    const REASON_RETURN_REQUESTED_BY_BENEFICIARY = 'RETURN_REQUESTED_BY_BENEFICIARY';
    const REASON_TERMS_AND_CONDITIONS_OF_ACCOUNT_DO_NOT_PERMIT_CREDITING_OF_THESE_FUNDS = 'TERMS_AND_CONDITIONS_OF_ACCOUNT_DO_NOT_PERMIT_CREDITING_OF_THESE_FUNDS';
    const REASON_SENDING_INSTITUTION_ACTION_REQUIRED = 'SENDING_INSTITUTION_ACTION_REQUIRED';
    const REASON_ACCOUNT_TRANSFERRED = 'ACCOUNT_TRANSFERRED';
    const REASON_REASON_NOT_SPECIFIED = 'REASON_NOT_SPECIFIED';
    ...
    {
        return [
            self::REASON_ACCOUNT_UNKNOWN,
            self::REASON_ACCOUNT_CLOSED,
            self::REASON_ACCOUNT_STOPPED,
            self::REASON_ACCOUNT_HOLDER_DECEASED,
            self::REASON_REFERENCE_REQUIRED_NOT_SUPPLIED,
            self::REASON_ACCOUNT_NAME_MISSMATCH,
            self::REASON_RETURN_REQUESTED_BY_SENDER,
            self::REASON_RETURN_REQUESTED_BY_BENEFICIARY,
            self::REASON_TERMS_AND_CONDITIONS_OF_ACCOUNT_DO_NOT_PERMIT_CREDITING_OF_THESE_FUNDS,
            self::REASON_SENDING_INSTITUTION_ACTION_REQUIRED,
            self::REASON_ACCOUNT_TRANSFERRED,
            self::REASON_REASON_NOT_SPECIFIED,
            self::REASON_OTHER,
        ];
    }
    ...
    {
        $this->container['paymentUidToReturn'] = $data['paymentUidToReturn'] ?? null;
    ...
    public function listInvalidProperties()
    {
        ...
        if ($this->container['reason'] === null) {
            $invalidProperties[] = sprintf('"%s" can\'t be null', 'reason');
        }
    ...
    /**
     * Gets paymentUidToReturn
     *
     * @return string|null
     */
    public function getPaymentUidToReturn()
    {
        return $this->get('paymentUidToReturn');
    }

    /**
     * Sets paymentUidToReturn
     *
     * @param string|null $paymentUidToReturn Unique identifier of the payment to return
     *
     * @return $this
     */
    public function setPaymentUidToReturn($paymentUidToReturn)
    {
        $this->container['paymentUidToReturn'] = $paymentUidToReturn;
    ...
    public function setReference($reference)
    {
        if ((mb_strlen($reference) > 18)) {
            throw new \InvalidArgumentException('invalid length for $reference when calling PaymentReturnRequest., must be smaller than or equal to 18.');
        }
        if ((mb_strlen($reference) < 1)) {
            throw new \InvalidArgumentException('invalid length for $reference when calling PaymentReturnRequest., must be bigger than or equal to 1.');
        }
        if ((! preg_match("/[a-zA-Z0-9-\/?:().,+#=!%&*<>;{@ \"']{1,18}/", $reference))) {
            throw new \InvalidArgumentException("invalid value for $reference when calling PaymentReturnRequest., must conform to the pattern /[a-zA-Z0-9-\/?:().,+#=!%&*<>;{@ \"']{1,18}/.");
        }
     ...
     * Sets reason
     *
     * @param string $reason Reason the payment is being returned
     *
     * @return $this
     */
